...
En primer lugar, amos a filtrar por los valores máximos de Peso, tabla, "X", "Y", "Z" y Volumen (variables que, como veremos más adelante, están fuertemenete correlacionadas).
...
Para ello, en primer lugar, vamos a dividir los datos en entrenamiento y test, 2/3 para entrenar y 1/3 para probar.

```{r}
library(gridExtra)
library(grid)

#Añadimos la columna de precio discretizado al dataset
diamonds_df_ext<-cbind(diamonds_df, cuartil.price)

#Desordenamos los datos que tenemos
diamonds_random <- diamonds_df_ext[sample(nrow(diamonds_df_ext)),]

#Dividimos la variable a clasificar y los parámetros de clasificación
y <- diamonds_random$cuartil.price 
x <- diamonds_random[,c(1,11)]

#Dividimos los datos de entrenamiento y test
indexes = sample(1:nrow(diamonds_random), size=floor((2/3)*nrow(diamonds_random)))
trainx<-x[indexes,]
trainy<-y[indexes]
testx<-x[-indexes,]
testy<-y[-indexes]
...
A continuación, vamos a crear el modelo, para ello, utilizaremos un árbol C.5
...
A continuación, vamos a validar el modelo con los datos reservados:
...
Como hemos visto, su precisión es del 78.5778%

Vamos a comprobar otro modelo, el incluído en la librería "RPART"

Aplicamos a continuación la función "RPART" con el método Clasificación para crear dicho árbol.

```{r}
library(rpart)
library(rpart.plot)

model_2<- rpart(trainx , trainy, cp=0.2)
```

Estudiamos las reglas aplicadas por el modelo.

```{r}
rpart.rules(model_2)
```

Y vemos una representación gráfica del árbol creado.

```{r}
rpart.plot(model_2)
```

De donde se exraen las siguiente conclusiones:
