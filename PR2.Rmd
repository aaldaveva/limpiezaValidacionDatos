...
Del mismo modo que la Spearman, la correlación de Kendall-Tau se basa en rangos.

# Descomentar para generar el PDF y el HTML, luego volver a comentar!
# ```{r}
# library (reshape2, warn.conflicts = FALSE)
# library(ggplot2)
# #Creamos una matriz de correlación con los datos descritos
# corr.mat<-round(cor(diamonds_df[,c(7,1,5,6,11)], method = "kendall"),2)
# #Visualizams los datos mediante un mapa de calor
# corr.mat[lower.tri(corr.mat)]<- NA
# m.corr.mat<-melt(corr.mat, na.rm=TRUE)
# # Fuente: http://www.sthda.com/english/wiki/ggplot2-quick-correlation-matrix-heatmap-r-software-and-data-visualization
...
Vemos su índice de correlación
 
# ```{r}
# library(psych)
# pairs.panels(diamonds_df[,c(7,11)], ellipses = FALSE, lm=TRUE, method = "kendall")
# pairs.panels(diamonds_df[,c(7,1)], ellipses = FALSE, lm=TRUE, method = "kendall")
# pairs.panels(diamonds_df[,c(11,1)], ellipses = FALSE, lm=TRUE, method = "kendall")
# ```

Obteniendo básicamente los mismos resultados para los dos análisis:
...
El problema es que como las variables "Peso" y "Volumen" tenían tanta relación entre ellas, el algoritmo sólo está tomando una de ellas, ya que las reglas serían parecidas para ambas variables.

Vamos a comprobar la bondad del árbol de clasificación utilizando los datos de test que habíamos reservado y los comparamos con los orignales mediante una matriz de confusión.
