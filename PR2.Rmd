...
corr.mat<-round(cor(diamonds_df[,c(8,2,6,7,12)], method = "pearson"),2)
...
```{r}
pairs.panels(diamonds_df[,c(8,12)], ellipses = FALSE, lm=TRUE, method = "pearson")
pairs.panels(diamonds_df[,c(8,2)], ellipses = FALSE, lm=TRUE, method = "pearson")
```
...
```{r}
chisq.test(cuartil.price$price, diamonds_df$cut)
